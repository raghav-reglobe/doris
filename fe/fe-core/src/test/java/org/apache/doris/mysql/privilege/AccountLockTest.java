...
public class AccountLockTest extends TestWithFeService {
    ...
    @Test
    public void testLockIsAnAuthenticationCheckNotASessionCheck() throws Exception {
        // MySQL proxy semantics: a PROXY_PRIV holder may still switch into a locked account;
        // the lock refuses the account's OWN logins only.
        run("CREATE USER 'lk_svc'@'%' IDENTIFIED BY 's'");
        run("CREATE USER 'lk5'@'%' IDENTIFIED BY 'p5'");
        run("CREATE ROLE r_lk5");
        run("GRANT 'r_lk5' TO 'lk5'@'%'");
        run("GRANT PROXY_PRIV ON *.*.* TO 'lk_svc'@'%'");
        run("ALTER USER 'lk5'@'%' ACCOUNT_LOCK");
        Assertions.assertFalse(canLogin("lk5", "p5"));
        Assertions.assertTrue(canLogin("lk_svc", "s"));

        ConnectContext ctx = new ConnectContext();
        ctx.setCurrentUserIdentity(ident("lk_svc"));
        ctx.setThreadLocalInfo();
        try {
            new SuUserCommand(new UserIdentity("lk5", "%"), Collections.singletonList("r_lk5"), null)
                    .run(ctx, null);
            Assertions.assertEquals("lk5", ctx.getCurrentUserIdentity().getQualifiedUser());
            Assertions.assertEquals("lk_svc", ctx.getAuthenticatedIdentity().getQualifiedUser());
        } finally {
            connectContext.setThreadLocalInfo();
        }
        // locking the SWITCHER is what stops the switch: its own login is refused
        run("ALTER USER 'lk_svc'@'%' ACCOUNT_LOCK");
        Assertions.assertFalse(canLogin("lk_svc", "s"));
    }
}
