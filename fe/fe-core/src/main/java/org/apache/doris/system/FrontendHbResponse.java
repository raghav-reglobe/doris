...
public class FrontendHbResponse extends HeartbeatResponse implements Writable {
    ...
    // Journaled: a follower learns another FE's build only from the replayed heartbeat (the master alone
    // runs heartbeats), and the SU forward gate compares builds on the follower - without this annotation
    // every follower held null here.
