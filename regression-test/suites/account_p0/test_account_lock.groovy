...
// USER, and -- like MySQL proxy authentication -- does not stop a PROXY_PRIV
// holder from switching into the locked account.
suite("test_account_lock", "account,nonConcurrent") {
    def user = "test_account_lock_user"
    def locked = "test_account_lock_born_locked"
    def svc = "test_account_lock_svc"
    def role = "test_account_lock_role"
    ...
    try_sql "DROP USER IF EXISTS '${svc}'@'%'"
    try_sql "DROP ROLE IF EXISTS ${role}"
    ...
    // 6. the lock is an authentication check, not a session check: a PROXY_PRIV
    //    holder can still switch into the locked account (MySQL proxy semantics)
    sql "CREATE USER '${svc}'@'%' IDENTIFIED BY 's'"
    grantClusterUsage(svc)
    ...
    assertFalse(canLogin(user, "p1"))
    connect(svc, "s", url) {
        sql "SU '${user}'@'%' WITH ROLES ('${role}')"
        def who = sql "SELECT current_user()"
        assertTrue(who[0][0].toString().startsWith("'${user}'"), who[0][0].toString())
    }
    // ... and locking the switcher is what stops the switch
    sql "ALTER USER '${svc}'@'%' ACCOUNT_LOCK"
    assertFalse(canLogin(svc, "s"))

    sql "DROP USER IF EXISTS '${user}'@'%'"
    sql "DROP USER IF EXISTS '${locked}'@'%'"
    sql "DROP USER IF EXISTS '${svc}'@'%'"
